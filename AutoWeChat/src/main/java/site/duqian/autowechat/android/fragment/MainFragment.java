...
public class MainFragment extends BaseSettingsFragment implements Preference.OnPreferenceChangeListener, Preference.OnPreferenceClickListener {
    ...
    private void openRobotRepley(boolean isOpen) {
        if (isOpen) {
            ToastUtils.showToast(context,context.getString(R.string.action_robot_repley_open));
        }else {
            ToastUtils.showToast(context,context.getString(R.string.action_robot_repley_close));
        }
    ...
    private void sendMessage() {
        if (checkService()) {
            SystemUtil.copyText(context,"本人正忙，稍后回复。。。");
